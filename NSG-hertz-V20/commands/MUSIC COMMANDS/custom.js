...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (args[0] === "add" || args[0] === "set" || args[0] === "use") {
      ...
      There are now: \`${playlist.length}\` songs in the Server playlist`)
    }
    if (args[0] === "reset" || args[0] === "res") {
      let themsg = await message.reply("Do you really want to reset your the Server playlist? ||(*Reply with:* **__`yes`__**)||")
      ...
      themsg.channel.awaitMessages(filter, {
        ...
        .then(async collected => {
          if (collected === "yes") {
            ...
            await message.reply("SUCCESSFULLY RESETTED THE SERVER CUSTOM PLAYLIST")
          }
        }).catch(error => {
    ...
    if (args[0] === "remove" || args[0] === "delete" || args[0] === "del" || args[0] === "rem") {
      ...
      return functions.embedbuilder(client, 5000, message, config.colors.yes, "SUCCESSFULLY REMOVED THE SONG FROM THE SERVER PLAYLIST")
    } else {
      let string = playlist.join("\n");
      customplay(message, string, playlist[0])
      functions.embedbuilder(client, "null", message, config.colors.yes, `There are ${playlist.length} Songs in the Server Playlist!`, )
      return functions.embedbuilder(client, "null", message, config.colors.yes, `Command Syntax:`, "+custom <add/remove/play> [Link]")
    }
  }
};
